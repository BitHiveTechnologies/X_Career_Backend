...
class EmailQueueService {
    constructor() {
        this.isInitialized = true; // Always initialized since no external dependencies
        this.stats = {
            totalProcessed: 0,
            totalFailed: 0,
            totalSucceeded: 0
        };
        logger_1.logger.info('Simple email queue service initialized (no Redis required)');
    }
    /**
    ...
    async addEmailJob(emailData, options) {
        try {
            ...
            logger_1.logger.info('Processing email immediately (no queue)', {
                jobId,
                to: emailData.to,
            ...
            while (attempts < maxAttempts && !success) {
                attempts++;
                try {
                    success = await emailService_1.emailService.sendEmail(emailData);
                    if (success) {
                        ...
                        return { id: jobId, status: emailService_1.EmailStatus.SENT };
                    }
                    else {
                        logger_1.logger.warn('Email sending failed, retrying...', {
                            jobId,
                            to: emailData.to,
                            attempt: attempts,
                            maxAttempts
                    ...
                    logger_1.logger.error('Email sending attempt failed', {
                        jobId,
                        to: emailData.to,
                        attempt: attempts,
            ...
            logger_1.logger.error('Email sending failed after all attempts', {
                jobId,
                to: emailData.to,
                attempts: maxAttempts
            });
            return { id: jobId, status: emailService_1.EmailStatus.FAILED };
        }
        catch (error) {
    ...
    async clearQueue() {
        ...
        logger_1.logger.info('Email queue stats cleared');
    }
    /**
     * Close queue (no-op for simple mode)
     */
    async closeQueue() {
        logger_1.logger.info('Email queue service closed (simple mode)');
    }
    /**
