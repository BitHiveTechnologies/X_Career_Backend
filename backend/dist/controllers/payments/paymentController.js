...
exports.handleWebhook = exports.getPaymentHistory = exports.verifyPayment = exports.createOrder = void 0;
...
const getPaymentHistory = async (req, res) => {
    try {
        ...
        const subscriptions = await Subscription_1.Subscription.find({ userId })
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limitNum);
        const total = await Subscription_1.Subscription.countDocuments({ userId });
        res.status(200).json({
            success: true,
            data: {
                subscriptions,
