...
jobSchema.methods.isAcceptingApplications = function(): boolean {
  return this.isActive && this.applicationDeadline > new Date();
};

// Instance method to get days until deadline
jobSchema.methods.getDaysUntilDeadline = function(): number {
  if (!this.isAcceptingApplications()) return 0;
...
jobSchema.methods.getCompensationDisplay = function(): string {
...
jobSchema.methods.isUserEligible = function(userProfile: any): boolean {
  // Check qualification
...
jobSchema.statics.findExpiringSoon = function(days: number = 7) {
...
jobSchema.virtual('deadlineStatus').get(function() {
  ...
  const daysUntilDeadline = (this as any).getDaysUntilDeadline();
