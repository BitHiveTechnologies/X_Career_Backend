...
jobNotificationSchema.methods.markEmailSent = function(): void {
  this.emailSent = true;
  this.emailSentAt = new Date();
...
jobNotificationSchema.methods.markEmailFailed = function(): void {
  this.emailSent = false;
  this.emailStatus = 'failed';
};

// Instance method to reset email status
jobNotificationSchema.methods.resetEmailStatus = function(): void {
  this.emailSent = false;
...
jobNotificationSchema.statics.findByEmailStatus = function(status: string) {
  return this.find({ emailStatus: status });
};

// Static method to find notifications that need retry
jobNotificationSchema.statics.findNotificationsForRetry = function(maxRetries: number = 3) {
  return this.find({
    emailStatus: 'failed',
    $expr: {
      $lt: [
...
jobNotificationSchema.statics.findByDateRange = function(startDate: Date, endDate: Date) {
  return this.find({
...
jobNotificationSchema.statics.getNotificationStatsByJob = function(jobId: string) {
  return this.aggregate([
...
jobNotificationSchema.virtual('timeSinceEmailSent').get(function() {
  return (this as any).getTimeSinceEmailSent();
});
...
jobNotificationSchema.virtual('notificationSummary').get(function() {
  return {
    ...
    isPending: (this as any).isPending(),
    wasSent: (this as any).wasSent(),
    hasFailed: (this as any).hasFailed()
  };
});
